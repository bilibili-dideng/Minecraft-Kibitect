...
from com.dideng.Python.MC_Kb.VMAV.mc_saver import MinecraftSaver
...
class MinecraftToolGUI:
    def __init__(self, root):
        ...
        # 加载主题配置
        try:
            theme_path = os.path.join(sys.path[1], "data", "theme", "VMAV","theme.json")

            with open(theme_path, "r", encoding="utf-8") as f:
                config = json.load(f)
                print("正在加载的主题文件:", os.path.abspath("./data/VMAV/theme.json"))
                STYLE.update(config)
            info(f"主题配置已从 JSON 加载: {config}")
        except Exception as e:
            warning(f"找不到主题文件，使用默认主题: {e}")
    ...
    def reload_style(self):
        ...
        try:
            with open(theme_path, "r", encoding="utf-8") as f:
                ...
                info(f"成功加载新样式: {STYLE}")
        except Exception as e:
            warning(f"加载新样式失败，使用当前样式继续运行: {e}")
    ...
    def load_world_info(self):
        ...
        try:
            ...
            info("成功获取世界信息、玩家坐标、背包数据")
        except FileNotFoundError:
            error("找不到 level.dat 文件！路径: %s", world_path, exc_info=True)
            messagebox.showerror("错误", "找不到 level.dat 文件！")
            return
        except Exception as e:
            error("加载世界信息时发生未知错误: %s", e)
    ...
    def export_to_json(self):
        ...
        try:
            ...
            info(f"数据已保存至：{file_path}")
            messagebox.showinfo("成功", f"数据已保存至：{file_path}")
        except Exception as e:
            error(f"导出 JSON 时发生错误: {e}")
...
def format_minecraft_time(tick):
    ...
    try:
        ...
        info(f"世界时间转换结果: {days} 天 {hours} 小时 {minutes} 分钟")
...
def format_real_time(timestamp_ms):
    """将毫秒时间戳转换为现实时间"""
    debug(f"转换现实时间: {timestamp_ms}")
    try:
        timestamp = int(timestamp_ms) / 1000
        dt = datetime.fromtimestamp(timestamp, tz=timezone.utc)
        info(f"现实时间转换结果: {dt.strftime('%Y-%m-%d %H:%M:%S')} UTC")
...
def roman_numeral(num):
    ...
    for value, symbol in val:
        while num >= value:
            res += symbol
            num -= value
    info(f"罗马数字转换结果: {res or 'I'}")
    return res or 'I'

def nbt_to_primitive(nbt_data, max_depth=32):
    """安全将 NBT 数据转换为 Python 原生类型"""
    debug(f"转换 NBT 数据: {repr(nbt_data)}")
    if max_depth <= 0:
        warning("NBT 转换超出最大递归深度")
        return "..."
